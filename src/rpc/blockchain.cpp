...
UniValue gettxout(const Config &config, const JSONRPCRequest &request) {
    if (request.fHelp || request.params.size() < 2 ||
        ...
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of Memcoin addresses\n"
...
static UniValue fillmempool(const Config &config, const JSONRPCRequest &request) {
    ...
    const auto destination_spk = CScript() << OP_TRUE;
    ...
    auto SpendToMempool = [&]
        ...
        while (tx.vout.size() < fanoutSize) {
            tx.vout.emplace_back(int64_t((amt_in / BIT) / fanoutSize) * BIT, destination_spk);
        }
        ...
        const auto fee = fee_rate.GetFee(tx_size) / BIT;
        const Amount fee_per_output = int64_t(std::ceil(fee / double(tx.vout.size() - n_op_returns))) * BIT;
        ...
        if (0 == tx_num % 1000 || last_fee_rate != fee_rate || tx_size > max_size_seen || tx_size < min_size_seen) {
            ...
            LogPrint(BCLog::MEMPOOL, "fillmempool: tx_num: %i, size: %i, fee: %i, fee_rate: %s\n",
                     tx_num, tx_size, tx_fee / BIT, fee_rate.ToString());
        }
