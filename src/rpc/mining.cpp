...
UniValue generateBlocks(const Config &config,
    ...
    while (nHeight < nHeightEnd && !ShutdownRequested()) {
        ...
        while (nMaxTries > 0 && pblock->nNonce < nInnerLoopCount &&
               !CheckProofOfWorkWithHeader(*pblock, pblock->nBits,
...
static UniValue prioritisetransaction(const Config &config,
        ...
        throw std::runtime_error(RPCHelpMan{
            ...
            "\nAccepts the transaction into mined blocks at a higher (or lower) priority.\n",
            {
                ...
                {"fee_delta", RPCArg::Type::NUM, /* opt */ false, /* default_val */ "",
                 "The fee value (in bits) to add (or subtract, if negative).\n"
    ...
    Amount nAmount = request.params[2].get_int64() * BIT;
...
static UniValue getblocktemplatecommon(bool fLight, const Config &config, const JSONRPCRequest &request) {
    ...
    if (!config.GetAllowUnconnectedMining()) {
        ...
        if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0) {
            throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED,
                               "Memcoin is not connected!");
        }
    }

    if (IsInitialBlockDownload()) {
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD,
                           "Memcoin is downloading blocks...");
    }
    ...
    if (fLight) {
        ...
        for (const auto &it : pblock->vtx) {
            ...
            entry.emplace_back("fee", pblocktemplate->entries[index_in_template].fees / BIT);
    ...
    result.emplace_back("coinbasevalue", pblock->vtx[0]->vout[0].nValue / BIT);
    ...
    if (request.fHelp || request.params.size() > 1) {
        ...
        throw std::runtime_error(RPCHelpMan{
            ...
            RPCResult{
                ...
                "         ],\n"
                "         \"fee\": n,                    (numeric) "
                "difference in value between transaction inputs and outputs (in bits); for coinbase transactions, "
    ...
    // first, write to file if file does not already exist
    {
        ...
        if (!fs::exists(outputFile)) {
            ...
            if (ok) {
                ...
                // We must throw here. Clients should be alerted that there is a misconfiguration with memcoind (even
