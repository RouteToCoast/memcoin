 ...
 *
 * This is the developer documentation of Memcoin Node


 * enables instant payments to anyone, anywhere in the world. Memcoin uses
...
static bool AppInit(int argc, char *argv[]) {
    ...
    try {
        ...
        extern bool g_pow_regtest;
        g_pow_regtest = Params().GetConsensus().fPowAllowMinDifficultyBlocks;
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                fprintf(stderr,
                        ...
                        "see memcoind -h for a list of options.\n",
                        argv[i]);
                return false;
            }
        }

        // -server defaults to true for memcoind but not for the GUI so do this
        // here
...
#endif
            fprintf(stdout, "Memcoin server starting\n");
...
int main(int argc, char *argv[]) {
    ...
    // Connect memcoind signal handlers
    noui_connect();
