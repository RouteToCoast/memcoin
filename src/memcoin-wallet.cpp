...
static bool WalletAppInit(int argc, char *argv[]) {
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        std::string usage =
            strprintf("%s memcoin-wallet version", PACKAGE_NAME) + " " +
                      FormatFullVersion() + "\n\n" +
                      "wallet-tool is an offline tool for creating and interacting with "
                      "Memcoin Node wallet files.\n" +
                      ...
                      "Usage:\n" + "  memcoin-wallet [options] <command>\n\n" +
...
int main(int argc, char *argv[]) {
    ...
    if (method.empty()) {
        std::fprintf(stderr, "No method provided. Run `memcoin-wallet -help` for "
                     "valid methods.\n");
        return EXIT_FAILURE;
    }
