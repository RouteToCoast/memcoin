...
static constexpr Amount DEFAULT_MIN_RELAY_TX_FEE_PER_KB(1000 * BIT);
...
//! Discourage users to set fees higher than this amount (in bits) per kB
static constexpr Amount HIGH_TX_FEE_PER_KB(COIN / 100);
/**
 * -maxtxfee will warn if called with a higher fee than this amount (in bits
 */
 ...
 * hours.
 */
static constexpr unsigned int DEFAULT_MEMPOOL_EXPIRY = 1080;
static constexpr int64_t DEFAULT_MEMPOOLMINVALUE = 10; // bit
/**
...
/**
 * Absolute maximum transaction fee (in bits) used by wallet and mempool
