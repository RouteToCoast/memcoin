    ...
    if (request.fHelp || request.params.size() > 2) {
        throw std::runtime_error(
            RPCHelpMan{"getnewaddress",
                "\nReturns a new Memcoin address for receiving payments.\n"
                ...
                .ToString() +
            "\nResult:\n"
            "\"address\"    (string) The new Memcoin address\n"
            "\nExamples:\n" +
    ...
    if (request.fHelp || request.params.size() > 1) {
        throw std::runtime_error(
            RPCHelpMan{"getrawchangeaddress",
                "\nReturns a new Memcoin address for receiving change.\n"
    ...
    if (request.fHelp || request.params.size() != 2) {
        throw std::runtime_error(
            RPCHelpMan{"setlabel",
                ...
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The Memcoin address to be associated with a label."},
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        throw std::runtime_error(
            RPCHelpMan{"sendtoaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The Memcoin address to send to."},
...
static UniValue listaddressgroupings(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 0) {
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"address\",            (string) The Memcoin address\n"
        ...
        throw std::runtime_error(
            RPCHelpMan{"signmessage",
                "\nSign a message with the private key of an address" +
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The Memcoin address to use for the private key."},
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            RPCHelpMan{"getreceivedbyaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The Memcoin address for transactions."},
...
static UniValue sendmany(const Config &config, const JSONRPCRequest &request) {
            ...
            RPCHelpMan{"sendmany",
                    ...
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    ...
                    {"amounts", RPCArg::Type::OBJ, /* opt */ false, /* default_val */ "", "A json object with addresses and amounts",
                        {
                            {"address", RPCArg::Type::AMOUNT, /* opt */ false, /* default_val */ "", "The Memcoin address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value"},
                        },
                    },
    ...
    for (auto &entry : sendTo) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                               std::string("Invalid Memcoin address: ") +
                                   entry.first);
        }
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        const std::string msg =
            RPCHelpMan{"addmultisigaddress",
                ...
                "Each key is a Memcoin address or hex-encoded public key.\n"
                ...
                "If 'label' is specified (DEPRECATED), assign address to that label.\n",
                {
                    ...
                    {"keys", RPCArg::Type::ARR, /* opt */ false, /* default_val */ "", "A json array of Memcoin addresses or hex-encoded public keys",
                        {
                            {"key", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "Memcoin address or hex-encoded public key"},
                        },
                        },
...
UniValue listtransactions(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() > 4) {
        throw std::runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"address\":\"address\",    (string) The Memcoin address of "
            "the transaction.\n"
...
static UniValue gettransaction(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "  \"details\" : [\n"
            "    {\n"
            "      \"address\" : \"address\",          (string) The Memcoin "
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            RPCHelpMan{"loadwallet",
                ...
                "\nNote that all wallet command-line options used when starting memcoind will be"
        ...
        throw std::runtime_error(
            RPCHelpMan{"listunspent",
                ...
                "Optionally filter to only include txouts paid to specified addresses.\n",
                {
                    ...
                    {"addresses", RPCArg::Type::ARR, /* opt */ true, /* default_val */ "", "A json array of Memcoin addresses to filter",
                        {
                            {"address", RPCArg::Type::STR, /* opt */ true, /* default_val */ "", "Memcoin address"},
                        },
                    },
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the Memcoin address\n"
            ...
            if (auto changeAddressUV = options_obj.locate("changeAddress")) {
                ...
                if (!IsValidDestination(dest)) {
                    throw JSONRPCError(
                        ...
                        "changeAddress must be a valid Memcoin address");
                }
                    ...
                    {"options", RPCArg::Type::OBJ, /* opt */ true, /* default_val */ "", "for backward compatibility: passing in a true instead of an object will result in {\"includeWatching\":true}",
                        {
                            {"include_unsafe", RPCArg::Type::BOOL, /* opt */ true, /* default_val */ RPCArg::Default(DEFAULT_INCLUDE_UNSAFE_INPUTS),
                             ...
                             "If that happens, you will need to fund the transaction with different inputs and "
                             "republish it."},
                            {"changeAddress", RPCArg::Type::STR, /* opt */ true, /* default_val */ "pool address", "The Memcoin address to receive the change"},
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            RPCHelpMan{"getaddressinfo",
                "\nReturn information about the given Memcoin address. Some information requires the address\n"
                "to be in the wallet.\n",
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The Memcoin address to get the information of."},
                }}
                .ToString() +
            "\nResult:\n"
            "{\n"
                ...
                "Implements the Creator and Updater roles.\n",
                {
                    ...
                    {"outputs", RPCArg::Type::ARR, /* opt */ false, /* default_val */ "", "a json array with outputs (key-value pairs)."
                            ...
                            "                             accepted as second parameter.",
                        {
                            {"", RPCArg::Type::OBJ, /* opt */ true, /* default_val */ "", "",
                                {
                                    {"address", RPCArg::Type::AMOUNT, /* opt */ true, /* default_val */ "", "A key-value pair. The key (string) is the Memcoin address, the value (float or string) is the amount in " + CURRENCY_UNIT + ""},
                                },
                                },
                    ...
                    {"options", RPCArg::Type::OBJ, /* opt */ true, /* default_val */ "", "",
                        {
                            {"include_unsafe", RPCArg::Type::BOOL, /* opt */ true, /* default_val */ RPCArg::Default(DEFAULT_INCLUDE_UNSAFE_INPUTS),
                             ...
                             "If that happens, you will need to fund the transaction with different inputs and "
                             "republish it."},
                            {"changeAddress", RPCArg::Type::STR_HEX, /* opt */ true, /* default_val */ "pool address", "The Memcoin address to receive the change"},
                            {"changePosition", RPCArg::Type::NUM, /* opt */ true, /* default_val */ "random", "The index of the change output"},
                            {"includeWatching", RPCArg::Type::BOOL, /* opt */ true, /* default_val */ "false", "Also select inputs which are watch only"},
                            {"lockUnspents", RPCArg::Type::BOOL, /* opt */ true, /* default_val */ "false", "Lock selected unspent outputs"},
                            {"feeRate", RPCArg::Type::AMOUNT, /* opt */ true, /* default_val */ "not set: makes wallet determine the fee", "Set a specific fee rate in " + CURRENCY_UNIT + "/kB"},
