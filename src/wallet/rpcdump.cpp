...
UniValue importaddress(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            RPCHelpMan{"importaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The Memcoin address (or hex-encoded script)"},
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                               "Invalid Memcoin address or script");
        }
    }
    if (fRescan) {
        RescanWallet(*pwallet, reserver);
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            RPCHelpMan{"dumpprivkey",
                ...
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The Memcoin address for the private key"},
                }}
                .ToString() +
            "\nResult:\n"
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            RPCHelpMan{"dumpwallet",
                ...
                {
                    {"filename", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The filename with path (either absolute or relative to memcoind)"},
                }}
                .ToString() +
            "\nResult:\n"
            "{                           (json object)\n"
                    ...
                    result.emplace_back(
                        ...
                        JSONRPCError(
                            ...
                            strprintf(
                                ...
                                "(see memcoind log for details) and could be "
