...
enum ServiceFlags : uint64_t {
    ...
    // chain. It is currently set by all Memcoin Node non pruned nodes, and is
    // unset by SPV clients or other light clients.
    NODE_NETWORK = (1 << 0),
    // NODE_GETUTXO means the node is capable of responding to the getutxo
    // protocol request. Memcoin Node does not support this but a patch set
    ...
    // connections. Memcoin Node nodes used to support this by default, without
    ...
    NODE_XTHIN = (1 << 4),
    // NODE_BITCOIN_CASH means the node supports Memcoin and the
    // associated consensus rule changes.
    // This service bit is intended to be used prior until some time after the
    // UAHF activation when the Memcoin network has adequately separated.
