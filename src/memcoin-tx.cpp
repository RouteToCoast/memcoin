...
static int AppInitRawTx(int argc, char *argv[], Config &config) {
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage =
            PACKAGE_NAME " memcoin-tx utility version " + FormatFullVersion() +
            "\n\n" +
            "Usage:  memcoin-tx [options] <hex-tx> [commands]  Update "
            "hex-encoded memcoin transaction\n" +
            "or:     memcoin-tx [options] -create [commands]   Create "
            "hex-encoded memcoin transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char *argv[], const Config &config, const CChainParams &chainParams) {
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded memcoin transaction
