...
#include <script/memcoinconsensus.h>
...
inline int set_error(memcoinconsensus_error *ret,
                     memcoinconsensus_error serror) {
    if (ret) {
        *ret = serror;
    }
...
static bool verify_flags(unsigned int flags) {
    return (flags & ~(memcoinconsensus_SCRIPT_FLAGS_VERIFY_ALL)) == 0;
}
...
static int verify_script(const uint8_t *scriptPubKey,
                         ...
                         memcoinconsensus_error *err) {
    if (!verify_flags(flags)) {
        return memcoinconsensus_ERR_INVALID_FLAGS;
    }
    try {
        TxInputStream stream(SER_NETWORK, PROTOCOL_VERSION, txTo, txToLen);
        CTransaction tx(deserialize, stream);
        if (nIn >= tx.vin.size()) {
            return set_error(err, memcoinconsensus_ERR_TX_INDEX);
        }

        if (GetSerializeSize(tx, PROTOCOL_VERSION) != txToLen) {
            return set_error(err, memcoinconsensus_ERR_TX_SIZE_MISMATCH);
        }

        // Regardless of the verification result, the tx did not error.
        set_error(err, memcoinconsensus_ERR_OK);
        ...
        // Error deserializing
        return set_error(err, memcoinconsensus_ERR_TX_DESERIALIZE);
    }
}

int memcoinconsensus_verify_script_with_amount(
    ...
    Amount am(amount * BIT);
    return ::verify_script(scriptPubKey, scriptPubKeyLen, am, txTo, txToLen,
                           nIn, flags, err);
}

int memcoinconsensus_verify_script(const uint8_t *scriptPubKey,
                                   unsigned int scriptPubKeyLen,
                                   const uint8_t *txTo, unsigned int txToLen,
                                   unsigned int nIn, unsigned int flags,
                                   memcoinconsensus_error *err) {
    if (flags & memcoinconsensus_SCRIPT_ENABLE_SIGHASH_FORKID ||
        flags & memcoinconsensus_SCRIPT_FLAGS_VERIFY_WITNESS_DEPRECATED) {
        return set_error(err, memcoinconsensus_ERR_AMOUNT_REQUIRED);
    }
...
unsigned int memcoinconsensus_version() {
