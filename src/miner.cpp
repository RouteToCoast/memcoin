...
BlockAssembler::CreateNewBlock(const CScript &scriptPubKeyIn, double timeLimitSecs, bool checkValidity,
    ...
    pblocktemplate->entries.emplace_back(CTransactionRef(), -BIT, -1);
...
void BlockAssembler::addTxs(int64_t nLimitTimePoint) {
    ...
    while (!TimedOut() && (!backlog.empty() || mi != mempool.mapTx.get<modified_feerate>().end())) {
        ...
        // Memcoin: TX1/TX2 are zero-fee by protocol, bypass blockMinFeeRate
        bool isMemcoinZeroFeeTx = false;
        {
            const CTransaction &tx = iter->GetTx();
            // TX1: has CLTV output
            for (const CTxOut &out : tx.vout) {
                if (std::find(out.scriptPubKey.begin(), out.scriptPubKey.end(),
                              uint8_t(OP_CHECKLOCKTIMEVERIFY)) != out.scriptPubKey.end()) {
                    isMemcoinZeroFeeTx = true;
                    break;
                }
            }
            // TX2: nLockTime > 0 and zero fee
            if (!isMemcoinZeroFeeTx && tx.nLockTime > 0 && iter->GetFee() == Amount::zero()) {
                isMemcoinZeroFeeTx = true;
            }
        }
        if (!isMemcoinZeroFeeTx && iter->GetModifiedFeeRate() < blockMinFeeRate) {
            break;
        }
        ...
        // Memcoin: TX1 CLTV range validation at pack time (R2)
        {
            const CTransaction &tx = iter->GetTx();
            for (const CTxOut &out : tx.vout) {
                auto it = std::find(out.scriptPubKey.begin(), out.scriptPubKey.end(),
                                    uint8_t(OP_CHECKLOCKTIMEVERIFY));
                if (it == out.scriptPubKey.end()) continue;
                CScript::const_iterator pc = out.scriptPubKey.begin();
                opcodetype op;
                std::vector<uint8_t> vch;
                if (!out.scriptPubKey.GetOp(pc, op, vch) || vch.empty()) break;
                int64_t cltvVal = 0;
                try { cltvVal = CScriptNum(vch, true, 5).getint64(); } catch (...) { break; }
                int64_t expected = (int64_t)nHeight + 432;
                if (std::abs(cltvVal - expected) > 10) {
                    goto skip_tx;
                }
                break;
            }
