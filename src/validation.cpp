...
const std::string strMessageMagic = "Memcoin Signed Message:\n";
...
AcceptToMemoryPoolWorker(const Config &config, CTxMemPool &pool,
...

    {
        ...
        // == Memcoin: Zero-Fee TX1/TX2 Payment System ==
        static const std::set<int64_t> MEMCOIN_STANDARD_DENOMS = {
            10000000LL, 1000000LL, 10000LL, 1000LL, 100LL, 10LL, 1LL
        };
        auto scriptHasCLTV = [](const CScript &s) -> bool {
            // Strict check: must match Memcoin CLTV script structure exactly
            // <height> OP_CLTV OP_DROP OP_DUP OP_HASH160 <20 bytes> OP_EQUALVERIFY OP_CHECKSIG
        ...
        auto getCLTVValue = [](const CScript &s, int64_t &val) -> bool {
            CScript::const_iterator pc = s.begin();
            opcodetype op; std::vector<uint8_t> vch;
            if (!s.GetOp(pc, op, vch) || vch.empty()) return false;
            opcodetype nextOp; std::vector<uint8_t> dummy;
            if (!s.GetOp(pc, nextOp, dummy)) return false;
            if (nextOp != OP_CHECKLOCKTIMEVERIFY) return false;
            try { val = CScriptNum(vch, true, 5).getint64(); return true; }
            catch (...) { return false; }
        };
        bool isTX1 = false;
        bool isTX2 = false;
        for (const CTxOut &out : tx.vout) {
            if (scriptHasCLTV(out.scriptPubKey)) { isTX1 = true; break; }
        }
        if (!isTX1 && tx.nLockTime > 0) {
            // TX2: ALL inputs must come from CLTV UTXOs (R6)
            int cltvInputCount = 0;
            int totalInputCount = (int)tx.vin.size();
            for (const CTxIn &txin : tx.vin) {
                // Check confirmed UTXOs
                const Coin &coin = view.AccessCoin(txin.prevout);
                if (!coin.IsSpent() && scriptHasCLTV(coin.GetTxOut().scriptPubKey)) {
                    cltvInputCount++; continue;
                }
                // Check mempool UTXOs (TX1 not yet confirmed)
                auto txEntry = pool.get(txin.prevout.GetTxId());
                if (txEntry) {
                    const CTxOut &mout = txEntry->vout[txin.prevout.GetN()];
                    if (scriptHasCLTV(mout.scriptPubKey)) {
                        cltvInputCount++; continue;
                    }
                }
            }
            if (cltvInputCount > 0 && cltvInputCount < totalInputCount) {
                // Mixed inputs: some CLTV, some not — explicit R6 violation
                return state.Invalid(false, REJECT_NONSTANDARD, "tx2-inputs-must-be-cltv",
                    strprintf("TX2: all inputs must be locked UTXOs (%d locked, %d non-locked)",
                              cltvInputCount, totalInputCount - cltvInputCount));
            }
            if (cltvInputCount == totalInputCount && totalInputCount > 0) isTX2 = true;
        }
        // TX1: zero-fee locked output creation
        if (isTX1) {
            if (nFees != Amount::zero())
                return state.Invalid(false, REJECT_NONSTANDARD, "tx1-zero-fee-required",
                    "TX1: zero fee required");
            for (const CTxOut &out : tx.vout) {
                if (!scriptHasCLTV(out.scriptPubKey)) continue;
                int64_t val = out.nValue / BIT;
                if (!MEMCOIN_STANDARD_DENOMS.count(val))
                    return state.Invalid(false, REJECT_NONSTANDARD, "tx1-invalid-denomination",
                        strprintf("TX1: invalid denomination %d bit (must be standard)", val));
                int64_t cltvVal = 0;
                if (!getCLTVValue(out.scriptPubKey, cltvVal))
                    return state.Invalid(false, REJECT_NONSTANDARD, "tx1-locktime-unparseable",
                        "TX1: locktime value unparseable");
                int64_t expected = (int64_t)::ChainActive().Height() + 1 + 432;
                if (std::abs(cltvVal - expected) > 30)
                    return state.Invalid(false, REJECT_NONSTANDARD, "tx1-locktime-out-of-range",
                        strprintf("TX1: locktime %lld out of valid range, expected ~%lld",
                                  cltvVal, expected));
            }
            // R4: rate limit — count CLTV UTXOs from the same source pkh
            //     across mempool + last 6 confirmed blocks
            {
                // Helper: extract pkh from P2PKH script, return empty if not P2PKH
                auto extractPKH = [](const CScript &s) -> std::vector<uint8_t> {
                    if (s.size() != 25) return {};
                    const uint8_t *d = s.data();
                    if (d[0]!=0x76||d[1]!=0xa9||d[2]!=0x14||d[23]!=0x88||d[24]!=0xac) return {};
                    return std::vector<uint8_t>(d+3, d+23);
                };
                // Helper: does script contain OP_CLTV?
                auto hasCLTV = [](const CScript &s) -> bool {
                    return std::find(s.begin(), s.end(),
                        uint8_t(OP_CHECKLOCKTIMEVERIFY)) != s.end();
                };

                for (const CTxIn &txin : tx.vin) {
                    // Resolve source scriptPubKey via pcoinsTip (confirmed coins only,
                    // bypasses mempool spent-marker so chained TX1s are visible)
                    CScript srcScript;
                    {
                        const Coin &coin = pcoinsTip->AccessCoin(txin.prevout);
                        if (!coin.IsSpent()) {
                            srcScript = coin.GetTxOut().scriptPubKey;
                        } else {
                            auto prevEntry = pool.get(txin.prevout.GetTxId());
                            if (prevEntry) srcScript = prevEntry->vout[txin.prevout.GetN()].scriptPubKey;
                        }
                    }
                    if (srcScript.empty()) continue;
                    std::vector<uint8_t> srcPKH = extractPKH(srcScript);
                    if (srcPKH.empty()) continue;

                    int cltvCount = 0;

                    // 1. Scan mempool via pcoinsTip to get true pkh (avoids spent-marker issue)
                    {
                        LOCK(pool.cs);
                        for (const auto &entry : pool.mapTx) {
                            ...
                            for (const CTxIn &vin : mtx.vin) {
                                ...
                                const Coin &c = pcoinsTip->AccessCoin(vin.prevout);
                                if (!c.IsSpent()) {
                                    pkh = extractPKH(c.GetTxOut().scriptPubKey);
                                } else {
                                    auto pe = pool.get(vin.prevout.GetTxId());
                                    if (pe) pkh = extractPKH(pe->vout[vin.prevout.GetN()].scriptPubKey);
                                }
                                if (pkh == srcPKH) { fromSrc = true; break; }
                            }
                            if (!fromSrc) continue;
                            for (const CTxOut &out : mtx.vout)
                                if (hasCLTV(out.scriptPubKey)) cltvCount++;
                        }
                    }

                    // 2. Scan last 6 confirmed blocks via pcoinsTip
                    const CBlockIndex *pindex = ::ChainActive().Tip();
                    for (int i = 0; i < 6 && pindex; i++, pindex = pindex->pprev) {
                        CBlock block;
                        if (!ReadBlockFromDisk(block, pindex, Params().GetConsensus())) continue;
                        for (const auto &btx : block.vtx) {
                            bool fromSrc = false;
                            for (const CTxIn &vin : btx->vin) {
                                const Coin &c = pcoinsTip->AccessCoin(vin.prevout);
                                if (c.IsSpent()) continue;
                                if (extractPKH(c.GetTxOut().scriptPubKey) == srcPKH) {
                                    fromSrc = true; break;
                                }
                            }
                            if (!fromSrc) continue;
                            for (const CTxOut &out : btx->vout)
                                if (hasCLTV(out.scriptPubKey)) cltvCount++;
                        }
                    }

                    if (cltvCount >= 1000)
                        return state.Invalid(false, REJECT_NONSTANDARD, "tx1-rate-limit-exceeded",
                            strprintf("TX1: rate limit exceeded %d/1000 CLTV UTXOs in 6-block window, source %s",
                                      cltvCount, txin.prevout.ToString()));
                }
            }
            // R3b: TX1 CLTV outputs must form the optimal (greedy) decomposition
            // of the total CLTV value. Prevents redundant splits (e.g. 23x1 instead of 10+10+1+1+1).
            {
                static const std::vector<int64_t> DENOM_ORDER = {
                    10000000LL, 1000000LL, 10000LL, 1000LL, 100LL, 10LL, 1LL
                };
                // Collect actual CLTV output denominations
                std::vector<int64_t> actualDenoms;
                int64_t totalCLTV = 0;
                for (const CTxOut &out : tx.vout) {
                    if (!scriptHasCLTV(out.scriptPubKey)) continue;
                    int64_t val = out.nValue / BIT;
                    actualDenoms.push_back(val);
                    totalCLTV += val;
                }
                // Compute greedy decomposition of totalCLTV
                std::vector<int64_t> greedyDenoms;
                int64_t rem = totalCLTV;
                for (int64_t d : DENOM_ORDER) {
                    while (rem >= d) { greedyDenoms.push_back(d); rem -= d; }
                }
                // Compare multisets (both vectors sorted for comparison)
                std::vector<int64_t> sortedActual = actualDenoms;
                std::sort(sortedActual.begin(), sortedActual.end());
                std::sort(greedyDenoms.begin(), greedyDenoms.end());
                if (sortedActual != greedyDenoms)
                    return state.Invalid(false, REJECT_NONSTANDARD, "tx1-denomination-not-optimal",
                        strprintf("TX1: denominations are not the optimal decomposition of %lld bit",
                                  totalCLTV));
            }
            // R9 removed: output count is naturally bounded by R3b greedy decomposition
            // Single recipient rule: all CLTV outputs must share the same pubkeyhash
            {
                std::vector<uint8_t> firstPKH;
                for (const CTxOut &out : tx.vout) {
                    if (!scriptHasCLTV(out.scriptPubKey)) continue;
                    // Extract pkh from script: skip locktime push + OP_CLTV + OP_DROP + OP_DUP + OP_HASH160 + 0x14
                    const uint8_t *d = out.scriptPubKey.data();
                    // locktime push length: first byte is push opcode
                    uint8_t first = d[0];
                    size_t ltEnd = (first >= 0x51 && first <= 0x60) ? 1 : 1 + first;
                    // pkh starts at ltEnd + 5 (OP_CLTV OP_DROP OP_DUP OP_HASH160 0x14)
                    const uint8_t *pkh = d + ltEnd + 5;
                    std::vector<uint8_t> thisPKH(pkh, pkh + 20);
                    if (firstPKH.empty()) {
                        firstPKH = thisPKH;
                    } else if (thisPKH != firstPKH) {
                        return state.Invalid(false, REJECT_NONSTANDARD, "tx1-single-recipient-only",
                            "TX1: all locked outputs must pay to a single recipient");
                    }
                }
            }
            // mempoolminvalue: TX1 total CLTV value must meet minimum threshold
            // R8: TX1 total CLTV output value <= 1 MEM
            Amount tx1TotalCLTV = Amount::zero();
            for (const CTxOut &out : tx.vout) {
                if (scriptHasCLTV(out.scriptPubKey)) tx1TotalCLTV += out.nValue;
            }
            {
                static const int64_t nMempoolMinValue =
                    gArgs.GetArg("-mempoolminvalue", DEFAULT_MEMPOOLMINVALUE);
                if (nMempoolMinValue > 0 && tx1TotalCLTV < nMempoolMinValue * BIT)
                    return state.Invalid(false, REJECT_NONSTANDARD, "tx1-min-value",
                        strprintf("TX1: total CLTV value %d bit below mempoolminvalue %d bit",
                                  tx1TotalCLTV / BIT, nMempoolMinValue));
            }
            if (tx1TotalCLTV > COIN)
                return state.Invalid(false, REJECT_NONSTANDARD, "tx1-amount-exceeds-limit",
                    strprintf("TX1: total CLTV value %d bit exceeds 1 MEM limit", tx1TotalCLTV / BIT));
            // Change output rule: if any P2PKH change output exceeds 1 MEM,
            // it must pay to the same pubkeyhash as the input (sender only).
            {
                // Extract sender pkh from first input
                auto extractPKH = [](const CScript &s) -> std::vector<uint8_t> {
                    if (s.size() != 25) return {};
                    const uint8_t *d = s.data();
                    if (d[0]!=0x76||d[1]!=0xa9||d[2]!=0x14||d[23]!=0x88||d[24]!=0xac) return {};
                ...
                std::vector<uint8_t> senderPKH;
                for (const CTxIn &txin : tx.vin) {
                    const Coin &coin = view.AccessCoin(txin.prevout);
                    if (!coin.IsSpent()) {
                        senderPKH = extractPKH(coin.GetTxOut().scriptPubKey);
                        if (!senderPKH.empty()) break;
                    }
                }
                for (const CTxOut &out : tx.vout) {
                    if (scriptHasCLTV(out.scriptPubKey)) continue;
                    if (out.nValue <= COIN) continue;
                    // P2PKH change > 1 MEM: must go back to sender
                    std::vector<uint8_t> changePKH = extractPKH(out.scriptPubKey);
                    if (changePKH.empty() || changePKH != senderPKH)
                        return state.Invalid(false, REJECT_NONSTANDARD, "tx1-change-must-return-to-sender",
                            "TX1: change output > 1 MEM must return to sender address");
                }
            }
        }
        // TX2: zero-fee locked output redemption
        if (isTX2) {
            if (nFees != Amount::zero())
                return state.Invalid(false, REJECT_NONSTANDARD, "tx2-zero-fee-required",
                    "TX2: zero fee required");
            for (const CTxIn &txin : tx.vin) {
                const Coin &coin = view.AccessCoin(txin.prevout);
                if (coin.IsSpent()) continue;
                const CScript &prev = coin.GetTxOut().scriptPubKey;
                if (!scriptHasCLTV(prev)) continue;
                int64_t cltvVal = 0;
                if (!getCLTVValue(prev, cltvVal))
                    return state.Invalid(false, REJECT_NONSTANDARD, "tx2-locktime-unparseable",
                        "TX2: source UTXO locktime unparseable");
                int64_t expected = cltvVal + 1;
                if (std::abs((int64_t)tx.nLockTime - expected) > 30)
                    return state.Invalid(false, REJECT_NONSTANDARD, "tx2-locktime-out-of-range",
                        strprintf("TX2: nLockTime %d out of valid range, expected ~%d",
                                  tx.nLockTime, expected));
            }

        }
        bool isMemcoinZeroFeeTx = isTX1 || isTX2;
        // == End Memcoin ==
        ...
        // policy upgrade.
        if (!bypass_limits && !isMemcoinZeroFeeTx && nModifiedFees < minRelayTxFee.GetFee(nSize)) {
...
Amount GetBlockSubsidy(int nHeight, const Consensus::Params &consensusParams) {
    int stage = nHeight / 157500;
    switch (stage) {
        case 0:  return 50 * COIN;
        case 1:  return 25 * COIN;
        case 2:  return 125 * COIN / 10;  // 12.5 MEM
        default: return 7 * COIN;
    }
}
...
bool CChainState::ConnectBlock(const CBlock &block, CValidationState &state,
    ...
    // Memcoin: skip PoW for blocks below the last checkpoint
    bool fCheckPoW = !fJustCheck;
    if (fCheckPoW && fCheckpointsEnabled) {
    ...
    if (!CheckBlock(block, state, consensusParams,
                    options.withCheckPoW(fCheckPoW)
    ...
    // Memcoin: skip script checks for blocks below last checkpoint
...
static bool CheckBlockHeader(const CBlockHeader &block, CValidationState &state,
    ...
    // Memcoin: use Yesmem hash via CheckProofOfWorkWithHeader
    if (validationOptions.shouldValidatePoW() &&
        !CheckProofOfWorkWithHeader(block, block.nBits, params)) {
...
bool CChainState::AcceptBlockHeader(const Config &config,
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        // Memcoin: skip PoW for header-only validation; full PoW checked on block download
        if (!CheckBlockHeader(block, state, chainparams.GetConsensus(),
                              BlockValidationOptions(config, false))) {
...
bool LoadMempool(const Config &config, CTxMemPool &pool) {
    ...
    try {
        ...
        while (num--) {
            ...
            Amount amountdelta = nFeeDelta * BIT;
