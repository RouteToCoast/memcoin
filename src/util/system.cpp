...
const char *const BITCOIN_CONF_FILENAME = "memcoin.conf";
...
public:
    ...
    static inline std::pair<bool, std::string> GetArg(const ArgsManager &am,
        ...
        // argument value seen from the command line (so "memcoind -foo=bar
...
bool ArgsManager::ParseParameters(int argc, const char *const argv[],
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; // memcoin-tx using stdin
        std::string val;
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Memcoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Memcoin
// Mac: ~/Library/Application Support/Memcoin
// Unix: ~/.memcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Memcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Memcoin";
#else
    // Unix
    return pathRet / ".memcoin";
#endif
#endif
}
...
const fs::path &GetDataDir(bool fNetSpecific) {
    ...
    if (fs::create_directories(path)) {
        ...
        // also remove the corresponding line in memcoind.cpp AppInit.
        // See more info at:
